  ...
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
		...
		<template slot-scope="scope">
		  <span style='color:red;'>{{scope.row.goodsId}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="图片id" prop="imageId" >
		<template slot-scope="scope">
		  <span style='color:red;'>{{scope.row.imageId}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="图片类型" prop="imageType" >
		<template slot-scope="scope">
		  <span style='color:red;'>{{scope.row.imageType}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.giId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.giId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <lnkgoodsimageform ref="lnkgoodsimageform" @save-ok="doQuery"></lnkgoodsimageform>

  </div>
</template>

<script>
  import lnkgoodsimageform from './lnkgoodsimageform'
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['lnkgoodsimageform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/lnkgoodsimage/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/lnkgoodsimage/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
