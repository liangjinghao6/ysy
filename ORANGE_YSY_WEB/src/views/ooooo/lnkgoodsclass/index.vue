<template>
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="类别id" prop="classId" >
		<template slot-scope="scope">
	...
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.gcId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.gcId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <lnkgoodsclassform ref="lnkgoodsclassform" @save-ok="doQuery"></lnkgoodsclassform>

  </div>
</template>

<script>
  import lnkgoodsclassform from './lnkgoodsclassform'
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['lnkgoodsclassform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/lnkgoodsclass/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/lnkgoodsclass/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
