<template>
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="退货单id" prop="backId" >
		<template slot-scope="scope">
		  <span>{{scope.row.backId}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.bgId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.bgId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <lnkbackgoodsform ref="lnkbackgoodsform" @save-ok="doQuery"></lnkbackgoodsform>

  </div>
</template>

<script>
  import lnkbackgoodsform from './lnkbackgoodsform'
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['lnkbackgoodsform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/lnkbackgoods/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/lnkbackgoods/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
