  ...
  <el-dialog title="编辑" :visible.sync="visiable" width='60%'>
    <el-form class="small-space" label-width="120px" :model="formInfo" ref="formInfo" :inline="true">
      <el-row justify="space-around">
        ...
        <el-col :span="12">
          <el-form-item label="订单id" prop="orderId" :rules="$r(1,0,36)">
            <el-input type="text" v-model="formInfo.orderId" placeholder="请输入订单id">
            </el-input>
          </el-form-item>
        </el-col>
  ...
  export default {
    ...
    methods: {
      show(id, method) {
        ...
        this.$http.get('/iweb/lnkordergoods/detail/' + id).then(response => {
      ...
      add() {
        this.loading = true
        this.$http.post('/iweb/lnkordergoods/add', this.formInfo).then(response => {
          this.loading = false
      ...
      update() {
        this.loading = true
        this.$http.post('/iweb/lnkordergoods/update', this.formInfo).then(response => {
          this.loading = false
          const data = response.data
          this.$message({
            type: data.status ? 'success' : 'error',
