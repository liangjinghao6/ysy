<template>
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="特征名称" prop="featureName" >
	...
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.featureId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.featureId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <goodsfeatureform ref="goodsfeatureform" @save-ok="doQuery"></goodsfeatureform>

  </div>
</template>

<script>
  import goodsfeatureform from './goodsfeatureform'
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['goodsfeatureform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/goodsfeature/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/goodsfeature/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
