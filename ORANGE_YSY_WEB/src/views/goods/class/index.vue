<template>
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="类别名称" prop="className" >
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="特征id" prop="featureId" >
		<template slot-scope="scope">
		  <span>{{scope.row.featureId}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
	...
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.classId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.classId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <goodsclassform ref="goodsclassform" @save-ok="doQuery"></goodsclassform>
...
<script>
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['goodsclassform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/goodsclass/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/goodsclass/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
