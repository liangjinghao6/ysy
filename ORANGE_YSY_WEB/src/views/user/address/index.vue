<template>
  <div class="app-container calendar-list-container">
	...
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="取货人姓名" prop="pickerName" >
		<template slot-scope="scope">
		  <span>{{scope.row.pickerName}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="取货人电话" prop="pickerPhone" >
		<template slot-scope="scope">
		  <span>{{scope.row.pickerPhone}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
          <el-table-column width="150px" align="center" label="取货人地址" prop="pickerAddress" >
		<template slot-scope="scope">
		  <span>{{scope.row.pickerAddress}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
            <el-table-column width="100px" align="center" label="是否默认地址" prop="isDefault" >
		<template slot-scope="scope">
		  <span>{{scope.row.isDefault=='0'?'×':'√'}}</span>
		</template>
	      </el-table-column>
        </el-col>
	<el-col :span="12">
	...
	<el-col :span="12">
      ...
      <el-table-column width="150px" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.addressId, 'update')">编辑
          </el-button>
          <el-button size="small" type="danger" @click="doRemove(scope.row.addressId)">删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <useraddressform ref="useraddressform" @save-ok="doQuery"></useraddressform>
...
<script>
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['useraddressform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/iweb/useraddress/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.get(`/iweb/useraddress/find?q=${this.queryInfo.query.q}&pageNum=${this.queryInfo.pageNum}&pageSize=${this.queryInfo.pageSize}`).then(response => {
