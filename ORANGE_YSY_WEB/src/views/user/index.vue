<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input style="width: 200px;" class="filter-item" v-model="hdQuery.q" placeholder="请输入姓名或账号"
    ...
    <el-table :data="hdResult.data" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column width="120px" align="center" label="姓名" prop="name" sortable>
        <template slot-scope="scope">
          <span style='color:red;'>{{scope.row.name}}</span>
        </template>
      </el-table-column>

      <el-table-column width="120px" align="center" label="登录账号" prop="account" sortable>
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="small" type="success" @click="doEdit(scope.row.id, 'password')">改密
          </el-button>
          <el-button size="small" type="success" @click="doEdit(scope.row.id, 'update')">编辑
          </el-button>
    ...
    methods: {
      ...
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          ...
          this.$http.get('/edi/user/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.post('/edi/user/find', this.hdQuery).then(response => {
