  ...
  <div class="app-container calendar-list-container">
    ...
    <el-col :span="16">
      <el-table :data="hdResult.data" v-loading="listLoading" border fit highlight-current-row
        ...
        <el-table-column width="120px" align="center" label="起始代码" prop="code" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.code}}</span>
          </template>
        </el-table-column>

        <el-table-column width="120px" align="center" label="位置" prop="pos" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.pos}}</span>
          </template>
        </el-table-column>
    ...
    methods: {
      ...
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          ...
          this.$http.get('/edi/unfixed/column/delete?id=' + id).then(response => {
      ...
      doQuery() {
        ...
        this.$http.post('/edi/unfixed/column/find', this.hdQuery).then(response => {
          const data = response.data.result
          this.hdResult = data
          this.listLoading = false
        })
        this.$http.get('/edi/edi/column/tree/' + this.tmpId).then(response => {
          const data = response.data.result.children
          this.hdTree = data
          this.listLoading = false
        })
      },
