  ...
  name: 'editform',
    ...
    methods: {
      show(id, method, tmpId) {
        this.method = method
        this.$http.get('/edi/edi/tmp/list/4').then(response => {
          const data = response.data.result;
          this.hdtemplate = data;
        })
        var hdquery = new HdQuery(this);
        hdquery.add("tmpId", tmpId);
        this.$http.post('/edi/data/column/list', hdquery).then(response => {
          const data = response.data.result;
          this.hdcolumn = data;
        })
        ...
        this.$http.get('/edi/data/column/detail/' + id).then(response => {
          const data = response.data.result
          this.hdform = data
        })
      ...
      add() {
        this.loading = true
        this.$http.post('/edi/data/column/add', this.hdform).then(response => {
          this.loading = false
      ...
      update() {
        this.loading = true
        this.$http.post('/edi/data/column/update', this.hdform).then(response => {
          this.loading = false
          const data = response.data
          this.$message({
            type: data.status ? 'success' : 'error',
