  ...
  <div class="app-container calendar-list-container">
    ...
    <el-col :span="16">
      <el-table :data="hdResult.data" v-loading="listLoading" border fit highlight-current-row
        ...
        <el-table-column width="120px" align="center" label="总长" prop="length" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.length}}</span>
          </template>
        </el-table-column>

        <el-table-column width="120px" align="center" label="起始位置" prop="startPos" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.startPos}}</span>
          </template>
        </el-table-column>
        <el-table-column width="120px" align="center" label="结束位置" prop="endPos" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.endPos}}</span>
          </template>
        </el-table-column>
    ...
    methods: {
      ...
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          ...
          this.$http.get('/edi/fixed/column/delete?id=' + id).then(response => {
      ...
      doQuery() {
        ...
        this.$http.post('/edi/fixed/column/find', this.hdQuery).then(response => {
