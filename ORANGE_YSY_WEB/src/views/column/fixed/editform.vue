  ...
  <el-dialog title="编辑" :visible.sync="visiable" width='60%'>
    <el-form class="small-space" label-width="120px" :model="hdform" ref="hdform" :inline="true">
      <el-row justify="space-around">
        ...
        <el-col :span="12" >
          <el-form-item label="长度" prop="length" :rules="[{ required: true, message: '此处不能为空',  trigger: 'blur' }]">
            <el-input type="text" v-model="hdform.length" placeholder="请输入起始代码">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" >
          <el-form-item label="起始位置" prop="startPos" :rules="[{ required: true, message: '此处不能为空',  trigger: 'blur' }]">
            <el-input type="text" v-model="hdform.startPos" placeholder="请输入位置">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" >
        <el-form-item label="终止位置" prop="endPos" :rules="[{ required: true, message: '此处不能为空',  trigger: 'blur' }]">
          <el-input type="text" v-model="hdform.endPos" placeholder="请输入位置">
          </el-input>
        </el-form-item>
      </el-col>
      </el-row>
  ...
  name: 'editform',
    ...
    methods: {
      show(id, method, tmpId) {
        this.method = method
        this.$http.get('/edi/edi/tmp/list/1').then(response => {
          const data = response.data.result;
          this.hdtemplate = data;
        })
        var hdquery = new HdQuery(this);
        hdquery.add("tmpId", tmpId);
        this.$http.post('/edi/fixed/column/list', hdquery).then(response => {
          const data = response.data.result;
          this.hdcolumn = data;
        })
        ...
        this.$http.get('/edi/fixed/column/detail/' + id).then(response => {
          const data = response.data.result
          this.hdform = data
        })
      ...
      add() {
        this.loading = true
        this.$http.post('/edi/fixed/column/add', this.hdform).then(response => {
          this.loading = false
      ...
      update() {
        this.loading = true
        this.$http.post('/edi/fixed/column/update', this.hdform).then(response => {
          this.loading = false
          const data = response.data
          this.$message({
            type: data.status ? 'success' : 'error',
