  ...
  <el-dialog title="编辑" :visible.sync="visiable" width='60%'>
    <el-form class="small-space" label-width="120px" :model="hdform" ref="hdform" :inline="true">
      <el-row justify="space-around">
        ...
        <el-col :span="12" >
          <el-form-item label="起始条件" prop="description" >
            <el-input type="text" v-model="hdform.regexCondiction" placeholder="请输入起始条件">
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" >
          <el-form-item label="匹配规则" prop="description" >
            <el-input type="text" v-model="hdform.regex" placeholder="请输入匹配规则">
            </el-input>
          </el-form-item>
        </el-col>
      </el-row>
  ...
  export default {
    ...
    methods: {
      show(id, method, tmpId) {
        this.method = method;
        this.$http.get('/edi/edi/tmp/list/5').then(response => {
          const data = response.data.result;
          this.hdtemplate = data;
        })
        var hdquery = new HdQuery(this);
        hdquery.add("tmpId", tmpId);
        this.$http.post('/edi/custom/column/list', hdquery).then(response => {
          const data = response.data.result;
          this.hdcolumn = data;
        })
        //弹窗重复使用时,清除校验项目
        ...
        this.$http.get('/edi/custom/column/detail/' + id).then(response => {
          const data = response.data.result
          this.hdform = data
        })
      ...
      add() {
        this.loading = true
        this.$http.post('/edi/custom/column/add', this.hdform).then(response => {
          this.loading = false
      ...
      update() {
        this.loading = true
        this.$http.post('/edi/custom/column/update', this.hdform).then(response => {
          this.loading = false
          const data = response.data
          this.$message({
            type: data.status ? 'success' : 'error',
