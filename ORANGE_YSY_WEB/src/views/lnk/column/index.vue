  ...
  <div class="app-container calendar-list-container">
    ...
    <el-col :span="16">
      <el-table :data="hdResult.data" v-loading="listLoading" border fit highlight-current-row
        ...
        <el-table-column width="120px" align="center" label="参数父级节点" prop="columnInParent" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.columnInParent.name}}</span>
          </template>
        </el-table-column>

        <el-table-column width="120px" align="center" label="输出结果节点" prop="columnOut" sortable>
          <template slot-scope="scope">
            <span style='color:red;'>{{scope.row.columnOut.name}}</span>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          ...
          this.$http.get('/edi/lnk/column/delete?id=' + id).then(response => {
    ...
    doQuery(tmpId) {
      ...
      doQuery() {
        ...
        this.$http.post('/edi/lnk/column/find', this.hdQuery).then(response => {
