<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :data="hdResult.data" v-loading="listLoading" border fit highlight-current-row
      ...
      <el-table-column width="120px" align="center" label="格式类型" prop="type" sortable>
        <template slot-scope="scope">
          <span>{{scope.row.ediType.name}}</span>
        </template>
      </el-table-column>

      <el-table-column width="120px" align="center" label="来源类型" prop="source" sortable>
        <template slot-scope="scope">
          <span>{{scope.row.ediSource != null ? scope.row.ediSource.name : '无'}}</span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      doEdit(id, method) {
        this.$refs['editform'].show(id, method)
      },
      doRemove(id) {
        this.$confirm('确定删除本数据?', '提示', {}).then(() => {
          this.listLoading = true
          this.$http.get('/edi/edi/tmp/delete?id=' + id).then(response => {
      ...
      doQuery() {
        this.listLoading = true
        this.$http.post('/edi/edi/tmp/find', this.hdQuery).then(response => {
