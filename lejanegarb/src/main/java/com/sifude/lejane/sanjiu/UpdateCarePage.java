...
/**
 * @author 梁景皓
 * @version 创建时间：2016年4月1日 上午11:11:42
 * 抓取某个网页相关联的page列表
 */
public class UpdateCarePage extends CrawlListPageBase{
	//请求头信息
	...
	public UpdateCarePage(String pageUrl) {
	...
	public List<String> getPageUrl(boolean exceptOther) {
		...
		if (exceptOther) {
			...
			for (String url : urls) {
				//url中是否包含"zongheng"
				if (url.indexOf("care.39.net/ys/jkjj/") > 0 && url.indexOf(".html") > 0) {
					excepUrls.add(url);
				}
			}
			return excepUrls;
	...
	public static void main(String[] args) {
		...
		UpdateCarePage updateList = new UpdateCarePage("http://care.39.net/ys/jkjj/index.html");
		for (String s : updateList.getPageUrl(true)) {
			System.out.println(s);
		}
		LeJanDB db = new LeJanDB();
		db.saveCrawlUrls(updateList.getPageUrl(true), "care", "page");
	}
}
